...
public class MazeTest extends JFrame implements GridColors {
 ...
 //       Blob aBlob = new Blob(aGrid);
    ...
    public void solve() {
        Maze maze = new Maze(theGrid);
        int i; 

        for (i = 0; i < maze.findAllMazePaths(0,0).size(); i++) {
        
        //prints all possible paths
        System.out.println("Path number " + i + ": " + maze.findAllMazePaths(0,0).get(i));
        }
        // prints shortest path
        System.out.println("Shortest Path: " + maze.findMazePathMin(0,0));

        maze.findAllMazePaths(0,0);
        
        maze.findMazePathMin(0,0);
        
        boolean found = maze.findMazePath();
